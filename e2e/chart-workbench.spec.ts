...
test('초기 화면은 기능 없는 흐름 버튼 없이 입력 대기 상태를 보여준다', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: /데이터를 불러오세요/ })).toBeVisible();
...
test('샘플 CSV를 선택하면 차트 후보와 대안 선택이 동작한다', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: '데이터를 어떤 형태로 볼까요?' })).toBeVisible();
...
test('파이와 도넛 차트는 휠 확대 대신 고정 비중 차트로 보여준다', async ({ page }) => {
  ...
  await expect(page.getByTestId('chart-static-viewport')).toBeVisible();
  ...
  const staticViewport = page.getByTestId('chart-static-viewport');
  await staticViewport.hover();
  await page.mouse.wheel(0, -500);
  await expect(page.getByTestId('chart-static-viewport')).toBeVisible();

  await page.getByRole('button', { name: /도넛 차트/ }).click();
  await expect(page.getByTestId('chart-static-viewport')).toBeVisible();
  await expect(page.getByTestId('chart-zoom-viewport')).toHaveCount(0);
});
...
test('차트 후보 캐러셀은 좌우 이동 버튼으로 넘길 수 있다', async ({ page }) => {
  ...
  await expect(choiceStrip.getByRole('button')).toHaveCount(7);
...
test('날짜 컬럼은 원본 흐름과 일·월·년 집계를 선택해 볼 수 있다', async ({ page }) => {
  ...
  await expect(page.getByRole('group', { name: '날짜 표시 방식' })).toBeVisible();
  await expect(page.getByRole('button', { name: '원본 순서' })).toHaveAttribute('aria-pressed', 'true');
  ...
  await expect(page.getByRole('group', { name: '날짜 집계 계산' })).toBeVisible();
  await expect(page.getByRole('button', { name: '합계' })).toHaveAttribute('aria-pressed', 'true');
  await expect(page.getByTestId('chart-svg').first().locator('circle')).toHaveCount(3);

  await page.getByRole('button', { name: '평균' }).click();
  await expect(page.getByRole('button', { name: '평균' })).toHaveAttribute('aria-pressed', 'true');
  await expect(page.getByTestId('chart-svg').first().locator('circle')).toHaveCount(3);

  await page.getByRole('button', { name: '연도별' }).click();
  await expect(page.getByRole('button', { name: '연도별' })).toHaveAttribute('aria-pressed', 'true');
...
test('대량 차트는 스크롤 줌을 명시적으로 켜서 사용할 수 있다', async ({ page }) => {
  ...
  await expect(page.getByTestId('chart-zoom-label')).toContainText('드래그 확대');

  await zoomViewport.hover();
  await page.mouse.wheel(0, -500);
  await expect(page.getByTestId('chart-zoom-range')).toHaveText('전체 120개');

  await page.getByRole('button', { name: '스크롤 줌', exact: true }).click();
...
test('대량 차트는 드래그로 특정 구간만 확대할 수 있다', async ({ page }) => {
  ...
  const box = await zoomViewport.boundingBox();
