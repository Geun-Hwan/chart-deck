...
test('키보드 입력과 오류 알림은 접근성 역할을 유지한다', async ({ page }) => {
  ...
  await expect(page.locator('.stage-topline strong')).toHaveText('월별 매출 CSV');

  await page.getByRole('button', { name: /CSV 텍스트 붙여넣기 열기/ }).click();
  await expect(page.getByRole('button', { name: /CSV 텍스트 붙여넣기 닫기/ })).toHaveAttribute('aria-controls', 'csv-paste-panel');
  await page.getByLabel('CSV 텍스트').fill('header-only');
